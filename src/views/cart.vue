        ...
        <div v-for="game in gamesgrid2" :key="game" class="bg-dark2 p-[1rem] mt-[1.2rem]
                    ...
                    <div class="flex flex-row  justify-between text-white">   
                        ...
                        <label v-if="game.dis" for="" class="text-white text-[.7rem] h-fit bg-blue1 rounded-md p-1 px-2 mr-3 cursor-pointer">{{game.dis}}</label>
                          <label v-if="game.oldpirce" for="" class="text-gray-500 mr-2 line-through cursor-pointer">${{ game.oldprice }}</label>
...
md:flex-row md:w-[78%] md:items-center `">
    ...
    <div class="flex items-start mt-[1rem] justify-start flex-col">
        ...
        <div class="flex mt-[1.5rem] flex-row">
            ...
            <label v-if="game.dis" for="" :class="`text-white text-[.7rem] bg-blue1 rounded-md p-1 px-2 mr-3 ${game.oldprice? '':'hidden'}`">{{ game.dis }}</label>
              <label v-if="game.oldprice" for="" :class="`text-gray-500 mr-2 line-through ${game.oldprice? '':'hidden'}`">${{ game.oldprice }}</label>
